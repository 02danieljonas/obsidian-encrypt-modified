...
export const VIEW_TYPE_ENCRYPTED_FILE_CONTENT =
    "meld-encrypted-file-content-view";
export class EncryptedFileContentView extends TextFileView {
    ...
    currentView: EncryptedFileContentViewStateEnum =
        EncryptedFileContentViewStateEnum.init;
    encryptionPassword = "";
    hint = "";
    ...
    constructor(
        ...
        this.defaultEditNoteView =
            (settings.defaultView as EditViewEnum) ?? EditViewEnum.source;
        this.currentEditNoteMode = this.defaultEditNoteView;

        this.elActionEditView = this.addAction(
            this.iconSourceView,
            "Source",
            () => this.toggleReadingView()
        );
        this.elActionReadingView = this.addAction(
            this.iconReadingView,
            "Reading",
            () => this.toggleReadingView()
        );
        this.elActionIconLockNote = this.addAction(
            this.iconLockFile,
            "Lock",
            () => this.actionLockFile()
        );
        this.elActionChangePassword = this.addAction(
            this.iconChangePassword,
            "Change Password",
    ...
    private async validatePassword(pw: string): Promise<string> {
        ...
        if (this.singlePassword && decryptedText == null) {
            console.debug(
    ...
    private addNewNoteView(container: HTMLElement) {
        ...
        new Setting(inputContainer).setDesc(
            "Please provide a password and hint to start editing this note."
        );
        ...
        const submit = async (
            ...
            const validPw = await this.validatePassword(password);
            ...
            if (validPw.length === 0 && validCpw.length === 0) {
                ...
                SessionPasswordService.putByFile(
        ...
        const bestGuessPassAndHint = SessionPasswordService.getByFile(
            this.file
        );
        ...
        const sPassword = UiHelper.buildPasswordSetting({
            ...
            onChangeCallback: async (value) => {
                password = value;
                sPassword.setDesc(await this.validatePassword(password));
        ...
        new Setting(inputContainer).addButton((bc) => {
            bc.setCta()
                .setIcon("go-to-file")
                .setTooltip("Edit")
    ...
    private addDecryptNoteView(container: HTMLElement) {
        ...
        new Setting(inputContainer).setDesc(
            "Please provide a password to unlock this note."
        );
        ...
        const sPassword = UiHelper.buildPasswordSetting({
            ...
            onChangeCallback: async (value) => {
                ...
                sPassword.setDesc(
                    await this.validatePassword(this.encryptionPassword)
                );
            },
        ...
        if (bestGuessPassAndHint.password.length > 0) {
            ...
            this.decryptWithPassword(bestGuessPassAndHint.password).then(
                (decryptedText) => {
    ...
    private addEditorReadingView(container: HTMLElement) {
        ...
        const readingContainer = container.createDiv({
            cls: "editor-reading-view",
        });
    ...
    private addChangePasswordView(container: HTMLElement) {
        ...
        const submit = async (
            ...
            const validPw = await this.validatePassword(newPassword);
            ...
            if (validPw.length === 0 && validCpw.length === 0) {
                ...
                SessionPasswordService.putByFile(
                    { password: newPassword, hint: newHint },
                    this.file
                );
        ...
        const sNewPassword = UiHelper.buildPasswordSetting({
            ...
            onChangeCallback: async (value) => {
                newPassword = value;
                sNewPassword.setDesc(await this.validatePassword(newPassword));
    ...
    private refreshView(newView: EncryptedFileContentViewStateEnum) {
    ...
    async handleDecryptButtonClick() {
        const decryptedText = await this.decryptWithPassword(
            this.encryptionPassword
        );
        ...
        if (decryptedText === null) {
            ...
            SessionPasswordService.putByFile(
                { password: this.encryptionPassword, hint: this.hint },
                this.file
            );
            this.currentEditorSourceText = decryptedText;
            this.refreshView(EncryptedFileContentViewStateEnum.editNote);
